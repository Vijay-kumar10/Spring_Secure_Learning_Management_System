...
public class UserResource {
	...
	public ResponseEntity<CommonApiResponse> registerUser(RegisterUserRequestDto request) {
		...
		//************************************send the verificaiton link to user ***********
		String emailToken = UUID.randomUUID().toString();
		user.setEmailToken(emailToken);

		existingUser = this.userService.addUser(user);
		String emailLink = EmailLinkHelper.getLinkForEmailVerification(emailToken);
		String mail_content = MailContentService.getVerifyMail(emailLink);
		emailService.sendEmail(existingUser.getEmailId(), "Verify Your Email to Complete LMS Registration", mail_content);
		...
		response.setResponseMessage("Verification Link Sent To Your Email");
		response.setSuccess(true);
	...
	///*********** Login With Google *******************
	public void googleLogin(UserLoginRequest userLoginRequest) {
		UserLoginResponse response = new UserLoginResponse();
		ResponseEntity<UserLoginResponse> response1 = login(userLoginRequest);
	}

	///***********  Login with form simple login *******************
	public ResponseEntity<UserLoginResponse> login(UserLoginRequest loginRequest) {
		...
		User user = this.userService.getUserByEmailid(loginRequest.getEmailId());
...
//		//check if accout is verified or not
		if(!Objects.equals(user.getEmailVerified(), "Verified")) {
		...
		if (user.getAccountLockedUntil() != null && user.getAccountLockedUntil().isAfter(LocalDateTime.now())) {
			String mail_Content = MailContentService.getAccountSuspensionMail();emailService.sendEmail(user.getEmailId(), "Account Temporarily Suspended Due to Multiple Failed Login Attempts", mail_Content);
		...
		try {
			...
			if (failedAttempts >= 3) {
				...
				user.setFailedAttempts(0); // reset for next time
				response.setResponseMessage("Account locked for 15 minutes due to multiple failed attempts");
			} else {
				response.setResponseMessage("Invalid email or password. Attempt " + failedAttempts + " of 3");
			}

			userDao.save(user); // save updated failedAttempts/accountLockedUntil
			response.setSuccess(false);
			return new ResponseEntity<>(response, HttpStatus.BAD_REQUEST);
		}
		...
		// ✅ Reset failedAttempts after successful login
		...
		if (jwtToken != null) {
			...
			String login_mail = MailContentService.getLoginMail();
			emailService.sendEmail(loginRequest.getEmailId(), "Successful Login to Your LMS Account", login_mail);
			UserController.loginOtp=null;
